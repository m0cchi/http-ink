...
(defun respond (env status content-type body &key (header '()))
  `(:header ,(remove nil `("HTTP/1.1" ,status
...
(defun respond-with-html (env text &key (charset "utf-8") (status "200 OK") (header '()))
  (respond env
           status
...
(defun respond-with-file (env file-path &key (status "200 OK") (header '()))
